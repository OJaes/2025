...
-- 그룹화 (GROUP BY) : 조회 결과에 대해 레코드를 하나 이상의 컬럼으로 그룹화하여 결과 도출
-- 일반적으로 집계함수와 같이 사용됨

SELECT employee_number, MAX(amount), MIN(amount) FROM sale GROUP BY employee_number;

-- 집계함수, 그룹화가 되어있는 쿼리에서는
-- 그룹화한 컬럼과 집계함수를 제외한 컬럼은 SELECT 절에서 선택 불가능

SELECT employee_number, date, MAX(amount), MIN(amount) FROM sale GROUP BY employee_number, date;

-- 필터링(HAVING) : 그룹화된 결과에 필터 작업을 수행
-- 결과 및 사용법이 WHERE 과 비슷하게 보이지만, WHERE은 조회할 때 사용, HAVING은 조회 후 사용됨
