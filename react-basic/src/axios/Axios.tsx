...
export default function Axios() {
    ...
    const onAgeChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        const { value } = event.target;
        const regexp = /^\d+$/;
        if (regexp.test(value)) setAge(value);
    };
    ...
    const onBasicGet = () => {
        axios
            ...
            // 응답이 존재하는 경우 http 상태 코드 400 ~ 500 처리
            // 응답이 존재하지 않는 경우도 처리리
            .catch((error) => {
    ...
    const onRequestBodyClickHandler = () => {
        ...
        // 객체 타입이어야 함
        //! post, patch, put 메서드의 두 번재 매개변수는 반드시 requestBody임
    ...
    return (
        <div>
            ...
            <div>
                <input value={name} placeholder={'이름'} onChange={onNameChangeHandler} />
                <input value={ho} placeholder={'호'} onChange={onHoChangeHandler} />
                <input value={age} placeholder={'나이'} onChange={onAgeChangeHandler} />
